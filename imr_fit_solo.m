...
%   parameters to match input data.
%
%   Only do NHKV model for now.
%   For now, only treat single experiment.
%
% =========================================================================
% References:
...
input_file = 'data/PA_05%_0.03%_WithRefined_T_c'; % 'data/PA_05%_0.03%_WithRefined_T_c'; 
load(input_file);

pick = 2; % Index of experiment to use
...
indx = find(t_all > 0, 1);
t_all = t_all(indx:end);
R_all = R_all(indx:end);
...
tcut = max_data(5,1)/tc; % max_data(3,1)/tc;  % Non-dimensional cut-off time
tspan_star = 1.5*tcut; % Simulate a little more for fitting purpose 
...
% search_sol = fminsearch(cost_fn,[G_start,mu_start]);
toc
...
plot_history(t_all,R_all,tS_dim,RS_dim,tcut*tc)
...
while ii < nX && min(nmin,nmax) < big
    ...
    if find_max == 0
        % Look for minimum
        slope = (RX((ii+1):end) - Rnow)./(tX((ii+1):end) - tnow); % sometimes we need to transpose ...
